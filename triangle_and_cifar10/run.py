...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            # ---------------- 控制最后一批次的保存开关 ----------------
            is_last_batch = (batch_idx == len(testloader) - 1)
            for m in net.modules():
                if m.__class__.__name__ == 'SW_LTM':
                    m.save_attention = is_last_batch
                    if is_last_batch:
                        m.attention_dict = {'hx1': [], 'hx2': []} # 清空旧数据
            # --------------------------------------------------------
            ...
            if is_last_batch:
                ...
                for m in net.modules():
                    if m.__class__.__name__ == 'SW_LTM':
                        for name, score_list in m.attention_dict.items():
                            if len(score_list) > 0:
                                # np.stack 会把包含4个矩阵的 list 转换成一个 5维大矩阵
                                # 维度变为: (Num_Layers, Batch_Size, Num_Heads, Query_Len, Key_Len)
                                stacked_scores = np.stack(score_list, axis=0)
                                np.save(f'attention_scores_{name}_all_layers.npy', stacked_scores)
                print(f"==> 成功保存测试集最后一个 Batch 的注意力权重，包含 hx1 和 hx2，各4层！")
            # ------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    decay_done = False
    #新增 CSV表头
    with open('accuracy_log.csv', 'w') as f:
        f.write('epoch,train_acc,test_acc\n')  # CSV表头
    #新增结束
    
    
    # 新增：如果是测试模式，跑完测试集直接退出
    if args.evaluate:
        print("==> Running evaluation on test set...")
        test_acc = test(start_epoch)
        import sys
        sys.exit(0)
