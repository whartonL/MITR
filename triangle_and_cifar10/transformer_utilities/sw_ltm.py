...
class SW_LTM(nn.Module):
    ...
    def multihead_attention(self, input, memory, use_topk_=True, store_log=True, save_name=None):
        """
        ...
        # ---------------- 新增收集逻辑 ----------------
        if getattr(self, 'save_attention', False) and save_name in ['hx1', 'hx2']:
            self.attention_dict[save_name].append(scores.detach().cpu().numpy())
        # ----------------------------------------------
    ...
    def forward_step(self, inputs, memory, Mr, treat_input_as_matrix=False):
        ...
        hx1 = self.multihead_attention(next_memory, inputs_reshape, use_topk_=False, store_log=False, save_name='hx1')
