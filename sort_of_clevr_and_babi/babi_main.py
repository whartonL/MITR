...
def train(args, force: bool = False) -> None:
    ...
    # -------------------------- 新增：初始化CSV文件 --------------------------
    # 生成不重复的CSV文件名（babi_result.csv → babi_result2.csv → ...）
    base_filename = "babi_result.csv"
    csv_filename = base_filename
    counter = 2
    while os.path.exists(csv_filename):
        csv_filename = f"babi_result{counter}.csv"
        counter += 1

    # 创建CSV文件并写入表头
    with open(csv_filename, mode='w', newline='', encoding='utf-8') as file:
        csv_writer = csv.writer(file)  # 重命名为csv_writer
        # 表头：Epoch + 各项指标
        csv_writer.writerow([  # 使用csv_writer
            "Epoch", 
    ...
    for i in range(1, trainer_config["epochs"]+1):
        ...
        with open(csv_filename, mode='a', newline='', encoding='utf-8') as file:
            ...
            csv_writer.writerow([  # 使用csv_writer
                i,  # 当前epoch（从0开始）
                round(train_acc, 5), round(train_loss, 5),
                round(valid_acc, 5), round(valid_loss, 5),
                round(test_acc, 5), round(test_loss, 5)
            ])
        # ----------------------------------------------------------------------------------
...
def test(args, force: bool = False) -> None:
    ...
    # --------- 新增：测试结束后保存 hx1 和 hx2 的注意力权重 ---------
    try:
        rmc = model.net.enc.self_attn.relational_memory
        if hasattr(rmc, 'hx1_scores_bf'):
            np.save('hx1_scores_bf.npy', np.array(rmc.hx1_scores_bf))
            np.save('hx1_scores_af.npy', np.array(rmc.hx1_scores_af))
            np.save('hx2_scores_bf.npy', np.array(rmc.hx2_scores_bf))
            np.save('hx2_scores_af.npy', np.array(rmc.hx2_scores_af))
            
            print(f"\n成功保存最后一个 Batch 的注意力权重！")
            print(f"hx1 数组形状 (TopK前后): {np.array(rmc.hx1_scores_bf).shape}")
            print(f"hx2 数组形状 (TopK前后): {np.array(rmc.hx2_scores_bf).shape}")
