...
class SimpleFlowMatching(nn.Module):
    def __init__(self, embed_dim, num_steps=5, dropout=0.1):
        super(SimpleFlowMatching, self).__init__()
    ...
    def forward(self, x):
        ...
        for step in range(self.num_steps):
            t_val = step * dt
            t = torch.full((seq_len, bsz, 1), t_val, device=x.device, dtype=x.dtype)
            
            x_t = torch.cat([x, t], dim=-1)
            v = self.net(x_t)
            
            # 欧拉步进：因为初始化 v 约等于 0，初始阶段 x 几乎无损通过
            x = x + v * dt 
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        args.use_wm_inference = use_wm_inference
    ...
    def forward(self, x, mask = None, num_layers = None):
        ...
        if not self.functional:
            # 1.functional = false, TR+HSW goes this route (In the case of only Transformer).
