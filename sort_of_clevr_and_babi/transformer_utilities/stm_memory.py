...
class STM(nn.Module):
    ...
    def compute(self, input_step, prev_state):  # input_step(64,256)
        ...
        controller_outp3 = self.input_projector3(input_step)  # (40->8)  (256->8)controller_outp3(128,8)

        # 2.Mi write 初级记忆的更新
        # 公式9 Bxdxd(64,256,256)  (64,128,1)*(64,1,128)=(B,slot_size,slot_size)
        X = torch.matmul(controller_outp.unsqueeze(2), controller_outp.unsqueeze(1))
        ...
        if self.rd:
            ...
            R = item_memory_state + X  # Bxdxd

        # 3.Mr read  关系记忆的读取
        # 使用 torch.einsum 函数执行张量乘法运算，通过使用合适的维度标签，我们可以自由地控制输入张量和输出张量的形状。
        controller_outp3 = F.softmax(controller_outp3, dim=-1)  # (128,8)
        # 公式11得到认知Vtr即controller_outp4  其维度为BD 与输入一样
        controller_outp4 = torch.einsum('bn,bd,bndf->bf', controller_outp3, controller_outp2,
                                        rel_memory_state)  # (128,128)
        # 4.Mr的更新  开始公式12: 公式12的一部分, Vtr与f2(xt)外积(让认知与输入相乘)
        X2 = torch.einsum('bd,bf->bdf', controller_outp4, controller_outp2)  # (128,128,128)
        for i in range(self.step):
            # 4.1 SAM操作  self.alpha2[i]*X2为全0, 形状不变
            qkv = self.qkv_projector[i](R + self.alpha2[i] * X2)  # 公式12的SAM()内容  D->N*3
            ...
            R = R + self.alpha3[i] * R2  # R为初步更新的Mi
        ...
        return out, (out, R, rel_memory_state)  # (隐状态, Mi, Mr)
