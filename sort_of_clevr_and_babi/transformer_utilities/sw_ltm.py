...
class SW_LTM(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_wm_inference = use_wm_inference
        ...
        # 1.2.3 Two normalization operations
        ...
        # Projecting the initial input for concatenation with memory to compute relational attention.
    ...
    def multihead_attention(self, input, memory, use_topk_=True, store_log=True, save_target=None):
        """
        ...
        # --------- 1. 保存 Top-K 之前的 scores ---------
        if not self.training:
            if save_target == 'hx1':
                self.hx1_scores_bf.append(scores.detach().cpu().numpy())
            elif save_target == 'hx2':
                self.hx2_scores_bf.append(scores.detach().cpu().numpy())
        # -----------------------------------------------
        ...
        # --------- 2. 保存 Top-K 之后的 scores ---------
        if not self.training:
            if save_target == 'hx1':
                self.hx1_scores_af.append(scores.detach().cpu().numpy())
            elif save_target == 'hx2':
                self.hx2_scores_af.append(scores.detach().cpu().numpy())
        # -----------------------------------------------       
    ...
    def forward_step(self, inputs, memory, Mr, treat_input_as_matrix=False):
        ...
        # 原代码：hx1 = self.multihead_attention(next_memory, inputs_reshape, use_topk_=False, store_log=False)
        hx1 = self.multihead_attention(next_memory, inputs_reshape, use_topk_=False, store_log=False, save_target='hx1')
        ...
        # 原代码：hx2 = self.multihead_attention(Mr_step, inputs_reshape)
        hx2 = self.multihead_attention(Mr_step, inputs_reshape, save_target='hx2')
        
        
        if self.long_mem_aggre:
            hx2 = self.multihead_attention(hx1, hx2, use_topk_=False, store_log=False)
        if self.use_wm_inference:
            hx = hx1 * 0.7 + hx2 * 0.3
        else:
            hx = hx2  # without WM
